...
def accumulate_languages(language_dictionaries):
    """ Calculate the total data size for each langauge """
    accumulated = defaultdict(int)
    total = 0
    for language_dictionary in language_dictionaries:
        for language_name, number_of_bytes in language_dictionary.items(): #no more dict.iteritems()
...
def get_language_dictionaries(repositories):
    """Return a list of dictionaries containing the languages used in each repo"""
...
def main():
    ...
    sorted_language_totals = sorted(language_totals.items(),    #no more dict.iteritems()
